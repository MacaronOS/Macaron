...
CreateBufferResult SharedBufferStorage::create_buffer(uint32_t size)
{
    ...
    uint32_t start_page = process.memory_description().allocate_memory_area<VMArea>(size, VM_READ | VM_WRITE).result()->vm_start() / CPU::page_size();
...
uint32_t SharedBufferStorage::get_buffer(uint32_t id)
{
    ...
    auto& process = Scheduler::the().current_process();
    uint32_t start_page = process.memory_description().allocate_memory_area<VMArea>(buffer->pages * CPU::page_size(), VM_READ | VM_WRITE).result()->vm_start() / CPU::page_size();
